...
- 大Key：key对应的value内存空间大（string过长或者集合数量过多）
- 热key：集中请求的Key

## 大key的问题

- 内存占用过高，导致内存碎片增加，性能下降，可能**阻塞**其他操作
- 获取大key时网络流量大，可能导致**带宽占用高**；同时传输大量数据，导致**主从同步延迟**
- ADB持久化中，阻塞写入硬盘（always发生，everysec不发生，因为异步执行）；RDB持久化中，增大页表，阻塞子进程的创建（因为父进程会复制一份页表）

## 大Key的发现和解决

- 将大Key分割为小Key，使用不同的命名空间；使用分布式存储
- 监控内存使用情况
- 定期清理过期数据
