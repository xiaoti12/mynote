...
此外，topic发生变化、订阅的topic中partition数量发生变化，也可能导致rebalance
...
- 消费者提交offset前宕机/重启
- 自动提交offset配置不当，消费后、提交offset前宕机
- 处理时间过长，超过`max.poll.interval`值，触发rebalance

解决措施：

- 禁用自动提交offset，改为手动提交（仍可能重复消费，提交offset前宕机）
- 优化处理速度，减少单次处理时间，避免reblance；或提高`max.poll.interval`值
- 引入接口幂等性，重复消费同一消息，也不会对系统状态产生影响

### 未消费/丢失

可能的原因：

- ack设置为0，消息在网络中丢失；设置为1，消费者消费前宕机
- broker将消息从page cache存储到磁盘前宕机
- 自动提交offset配置不当，未成功消费就提交了offset
...
解决办法：
- 控制消费时间，如果有长耗时逻辑则尽量异步处理
- 控制和外部服务交互，避免其他服务问题导致消费能力下降
- 及时扩容partition和消费者机器
